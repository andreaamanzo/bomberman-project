...
concreto i principi della programmazione orientata agli oggetti, con particolare
attenzione alla progettazione di un'architettura modulare, composta da
componenti con responsabilità ben definite e debolmente accoppiate.
...
\begin{itemize}
    ...
    \item \textbf{Menu}: gestisce le schermate iniziali del gioco e permette
...
le funzioni relative alla gestione delle bombe sono state implementate principalmente da Andrea Manzo, 
...
\subsection{Approfondimento del linguaggio C++}
...
\subsection{Ottimizzazione e correzione del codice}
...
\begin{minted}[fontsize=\small]{cpp}
void ScoreBoard::pushOrderly(const char* playerName, const int score) // push di un elemento in ordine decrescente di punteggio
{
  Node* foo = new Node;
...
\subsection{Tecniche comuni di approccio a problemi noti}
...
di una libreria poco familiare o per comprendere pattern progettuali applicabili al
contesto specifico. In altri casi sono stati utilizzati come interlocutori tecnici,
utili per discutere scelte architetturali e valutarne le implicazioni prima di
procedere all'implementazione. In altri ancora, il supporto è stato più operativo,
orientato alla risoluzione di problemi concreti emersi durante lo sviluppo.
...
Il metodo \texttt{shouldMove()} verifica se è trascorso il tempo minimo dal frame
precedente, permettendo a ogni nemico di avere una velocità indipendente:
...
Ogni sprite è definito come un piccolo blocco di caratteri testuali che viene
stampato nel terminale nella posizione corrispondente all'entità.
 
Ogni tipo di nemico ha uno sprite statico dedicato, selezionato al momento della
costruzione tramite la funzione:
 
\begin{minted}[fontsize=\small]{cpp}
const Nc::Sprite2x3& Enemy::getSprite(Type enemyType)
{
    switch (enemyType) {
    case Type::First_Enemy:  return s_enemy1;
    case Type::Second_Enemy: return s_enemy2;
    case Type::Third_Enemy:  return s_enemy3;
    default:                 return s_spriteTypeNull;
    }
}
\end{minted}
 
Il problema principale era trovare combinazioni di caratteri Unicode che, nella
griglia 2$\times$3 (2 righe, 3 colonne) del terminale, producessero una figura
stilizzata riconoscibile come un personaggio nemico. La scelta dei caratteri giusti
richiede una certa conoscenza del repertorio Unicode e della loro resa effettiva nel
terminale, che non è sempre prevedibile senza prova diretta.
...
Il tipo è specificato già al momento della costruzione dell'oggetto \texttt{Bomb},
con \texttt{Bomb::Type::Enemy} per le bombe dei nemici e \texttt{Bomb::Type::Player}
per quelle del giocatore, e influenza anche la grafica: il costruttore di \texttt{Bomb}
seleziona uno sprite diverso in base al tipo, rendendo visivamente distinguibili i
due casi durante il gioco.
...
Il messaggio di errore, pur contenendo in sé il suggerimento alla soluzione, non
era immediatamente chiaro riguardo alle implicazioni delle diverse opzioni. Ho
quindi utilizzato il modello per comprendere la differenza tra le tre strategie
proposte, con particolare attenzione alle conseguenze pratiche nel contesto di
un progetto collaborativo.
 
Il modello ha spiegato la differenza tra le tre strategie. L'opzione
\texttt{pull.rebase false} utilizza una strategia di \textit{merge}, creando un
commit aggiuntivo che unisce le due storie divergenti. L'opzione
\texttt{pull.rebase true}, invece, applica i propri commit locali \textit{sopra}
quelli remoti appena scaricati, come se fossero stati scritti partendo già dallo
stato aggiornato del repository: in questo modo la storia rimane lineare e i propri
cambiamenti risultano sempre in cima, senza commit di merge intercalati.
 
Ho scelto la strategia di \textit{rebase} proprio per questo motivo: lavorando in
...
Questa configurazione, impostata una volta sola a livello locale, risolve
definitivamente il problema rendendo il comportamento di \texttt{git pull}
esplicito e prevedibile per tutte le operazioni successive.
 
Nei casi in cui il rebase automatico non era possibile a causa di modifiche
conflittuali sullo stesso file, il modello a suggerito tramite editor di testo
di modificare a mano i conflitti generati.
